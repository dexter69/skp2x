    ...
    url: function(){
        ...
        */);
        ...
        return theResponse.then(function(data, text, XmlHttpRequest) {
            ...
            let validJSONstring = true;

            // W data.text() jest odpowiedź serwera
            try {
                // próbujemy skonwertować na json object
                JSON.parse(data.text());
            }
            catch(err) {
                validJSONstring = false; // nieprawidłowy
                console.log(err);
            }
            console.log("################################  ");  
            if( validJSONstring ) { 
                ...
                // Zakładamy, że skoro był nie prawidłowy, to uż nie jest zalogowany
