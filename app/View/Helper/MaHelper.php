...
class MaHelper extends AppHelper
{
	...
	private function getAndSetCechyKarty($karta = array())
	{
		...
		if (!empty($karta)) {
			...
			if ($karta['isperso']) {
				...
				if (
					!$this->tablica_cech['pl'] && !$this->tablica_cech['pt'] &&
	...
	public function cechyKarty($karta = array(), $wrap_klasa = null)
	{
		...
		if ($this->tablica_cech['isperso']) {
			if ($this->tablica_cech['pp']) { // karta po staremu
				...
				if ($this->tablica_cech['pl']) {
					$html .= $this->markCechy('L', 'p-lam', $this->ztitles['p-laminat']);
				}
				if ($this->tablica_cech['pt']) {
					$html .= $this->markCechy('P', 'p-pla', $this->ztitles['p-plaska']);
				}
				if ($this->tablica_cech['pe']) {
					$html .= $this->markCechy('E', 'p-emb', $this->ztitles['p-emboss']);
				}
			}
			$klasa = 'cechy-wrap';
		}
	...
	public function md($dstring, $short = false)
	{ //moja data
	...
	public function mdvs($dstring)
	{ //moja data bardzo krótka
	...
	public function mdt($dstring = null, $seconds = false)
	{ //moja data i czas
	...
	public function kontrolka($card, $evcontrol)
	{
		...
		if ($evcontrol['ile']) { //wyświetlamy formularz, jeżeli są w ogle jakieś submity
			...
			foreach ($buttons as $val) {
				if ($bcontr[$val]) $thecl = 'submit';
				else $thecl = 'submit nieaktywny';
	...
	public function kontrolka_ord($order, $evcontrol, $przypominajka = false)
	{
		...
		if ($evcontrol['ile']) { //wyświetlamy formularz, jeżeli są w ogle jakieś submity
			...
			foreach ($evcontrol['buttons'] as $val) {
				if ($evcontrol['bcontr'][$val]) $thecl = 'submit';
				else $thecl = 'submit nieaktywny';
	...
	public function kontrolka_job($job, $submity)
	{
		...
		function plikoza($przyciski = array())
		{
			...
			if (!empty($przyciski)) {
				if (
					array_key_exists(eJ_FILE1, $przyciski) ||
		...
		if (count($submity)) {
			...
			foreach ($submity as $key => $val) {
				...
				if ($key == eJ_FILE1 || $key == eJ_FILE2) $plik = 1;
				else $plik = 0;
	...
	public function ordersInJob($data)
	{
		...
		foreach ($data as $row) {
			...
			if ($row['Card']['job_id']) $checked = true;
			else $checked = false;
			...
			$utech = array(
				'<p id="wyz' . $cid . '" class="wyzwalacz"></p><div class="' . $klastr . '">' . $this->Form->input(
					'Card.' . $k . '.tech_comment',
					array(
	...
	public function klasaDolara($formaZaliczki = null, $stanZaliczki = null)
	{
		...
		if ($formaZaliczki > 1) { // jest zaliczka
			switch ($stanZaliczki) {
				case null:
					$klasa_dol = 'dolar red';
					break; // brak jakiegokolwiek wpisu
				case 'confirmed':
					$klasa_dol = 'dolar ora';
					break; // potwierdzona wpłata
				case 'money':
					$klasa_dol = 'dolar gre';
					break; // są pieniądze na koncie
			}
		} else {
			$klasa_dol = 'dolar gre';
		}
		...
		if (!empty($wspolne)) {
			...
			foreach ($wspolne as $value) {
				...
				$trs .= '<td>' . $this->Html->link($value['filename'],	array(
					'controller' => 'uploads', 'action' => 'download', $value['id']
				)) . '</td>';
